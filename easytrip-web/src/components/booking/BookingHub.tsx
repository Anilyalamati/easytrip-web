...
export const BookingHub: React.FC = () => {
  ...
  return (
    <div className="my-8 space-y-8">
      {/* Header */}
      <div className="glass-card rounded-md p-6 sm:p-8 border border-navy-750 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
        ...
        <button
          ...
          className="relative px-5 py-3 rounded-sm gold-gradient-bg text-navy-950 font-bold text-xs tracking-wide shadow-gold-glow hover:brightness-110 active:scale-95 transition-all flex items-center gap-2"
        >
      ...
      <div className="flex items-center gap-2 border-b border-navy-800 pb-3">
        <button
          ...
          className={`px-5 py-2.5 rounded-sm text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'hotels'
      ...
      {activeTab === 'hotels' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {options.hotels.map(hotel => {
            ...
            return (
              <div key={hotel.id} className="glass-card rounded-md overflow-hidden border border-navy-750 flex flex-col justify-between glass-card-hover">
                <div>
                  <div className="relative h-48 w-full overflow-hidden">
                    <img src={hotel.image} alt={hotel.name} className="w-full h-full object-cover" />
                    <span className="absolute top-3 left-3 px-2.5 py-1 rounded-sm bg-navy-900/90 text-[10px] font-bold text-gold-300 border border-gold-500/30 uppercase">
                      {hotel.badge}
                    </span>
                    <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 rounded-sm bg-navy-900/90 text-xs font-bold text-white">
                  ...
                  <div className="p-5 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-2">
                      {hotel.amenities.map(a => (
                        <span key={a} className="text-[10px] px-2 py-0.5 rounded-sm bg-navy-800 text-slate-300 border border-navy-700">
                          {a}
                        </span>
                      ))}
                    </div>
      ...
      {activeTab === 'transport' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {options.transport.map(tr => {
            ...
            return (
              <div key={tr.id} className="glass-card rounded-md p-6 border border-navy-750 flex flex-col justify-between space-y-4 glass-card-hover">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="px-2.5 py-1 rounded-sm bg-navy-850 text-gold-300 border border-gold-500/30 text-[10px] font-bold uppercase">
                      {tr.badge}
                    </span>
                  ...
                  <div className="my-4 p-3 rounded-sm bg-navy-900/90 border border-navy-750 flex items-center justify-between text-xs">
                    <div>
      ...
      {activeTab === 'experiences' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {options.experiences.map(exp => {
            ...
            return (
              <div key={exp.id} className="glass-card rounded-md overflow-hidden border border-navy-750 flex flex-col justify-between glass-card-hover">
                <div>
