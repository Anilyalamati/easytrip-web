...
export const EmergencyModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-md p-6 sm:p-8 shadow-elevated my-8">
        
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-sm bg-safety-50 border border-safety-200 flex items-center justify-center p-2 text-safety-600">
            ...
            <div>
              <h2 className="text-xl font-bold text-charcoal-900 tracking-tight flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-sm bg-safety-50 text-safety-700 border border-safety-200">
                  Priority 24/7
                </span>
              </h2>
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {emergencyContacts.map((contact, idx) => (
              <div
                key={idx}
                className="p-4 rounded-sm bg-slate-50 border border-slate-200 flex flex-col justify-between space-y-2 hover:border-safety-300 transition-all"
              >
                <div>
                ...
                <a
                  ...
                  className="flex items-center justify-between px-3 py-1.5 rounded-sm bg-safety-600 hover:bg-safety-700 text-white text-xs font-bold transition-all shadow-sm"
                >
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="p-4 rounded-sm bg-slate-50 border border-slate-200 space-y-2 text-xs text-charcoal-700">
        ...
        <div className="mt-6">
          <button
            onClick={() => setIsSosOpen(false)}
            className="w-full py-3 rounded-sm bg-slate-100 hover:bg-slate-200 text-charcoal-800 font-bold text-xs border border-slate-300/80 transition-all"
          >
